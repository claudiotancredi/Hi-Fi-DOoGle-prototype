...
function PostDetailsProfile(props) {
    ...
    const submitCommentLine = (e) => {
        ...
        API.addNewComment(newComment).then(() => { setDeleted(false); setEdited(false); props.setDeleted(false); props.setEdited(false); props.setEditedDetails(false); setFlagCommented(true); props.setDirty(true); })
        props.reset();
    };
    ...
    const handleNewSaved = () => {
        if (saved.some(e => e.postID === props.postDetails.ID)) {
            ...
            API.DeleteAssociate('SAVED', postID, props.postDetails.ID).then(() => {
                if (props.favorites) {
...
export default PostDetailsProfile;