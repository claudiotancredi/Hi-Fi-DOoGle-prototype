...
function FavoritePostsCopy(props) {
    ...
    useEffect(() => {
        const getPosts = () => {
            if (dirty) {
                API.getSavedInfo(props.userInfo.id)
                    .then(ps => { setPosts(ps) })
                    .catch(() => { setPosts([]); })
            ...
            {(!showPostDetails && !newPost) &&
                <>
                    <Container fluid className="profilepage px-0">
                        ...
                        <p className="nomatch2 centermatch4">Sorry, you haven't marked any</p>
                        <p className="nomatch2 centermatch3">posts as favorite yet.</p>
                        </>:<ListOfPostsProfile
